...
const serverlessConfiguration: AWS = {
  ...
  provider: {
    ...
    vpc: process.env.IS_OFFLINE === 'true' ? undefined : {
  ...
  custom: {
    ...
    dbPassword: process.env.IS_OFFLINE === 'true' ? 'rootpassword' : '${env:DB_PASSWORD}',
    dbName: process.env.IS_OFFLINE === 'true' ? 'EventSpark' : '${env:DB_NAME}',
    dbPort: process.env.IS_OFFLINE === 'true' ? '3306' : '${env:DB_PORT}',
    dbSsl: process.env.IS_OFFLINE === 'true' ? '{"rejectUnauthorized": false}' : '{"rejectUnauthorized": true}',
    cookieDomain: process.env.IS_OFFLINE === 'true' ? 'localhost' : '${env:COOKIE_DOMAIN}',
    frontendUrl: process.env.IS_OFFLINE === 'true' ? 'http://localhost:3000' : '${env:FRONTEND_URL}',
    jwtSecret: process.env.IS_OFFLINE === 'true' ? 'local-development-secret' : '${env:JWT_SECRET}',
    esbuild: {
      bundle: true,
      minify: false,
      sourcemap: true,
